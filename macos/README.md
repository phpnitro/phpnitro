...
`MacScreenViewController.swift`/`MacCanvasView.swift` (chemin Core Graphics) gagnent la MÊME détection de redimensionnement (`MacCanvasView.onResize`, câblé via Auto Layout — le pin aux bords du conteneur suffit à propager un redimensionnement de fenêtre) et le MÊME overlay `focus:` (`MacCanvasView.showTextInput`, `PhpNitroProtocol.DrawCommandPayload.region(at:)` ajouté pour donner le rect du hit à `onAction`) — les deux seuls changements apportés à ce chemin déjà éprouvé, sans toucher au rendu Core Graphics lui-même.
