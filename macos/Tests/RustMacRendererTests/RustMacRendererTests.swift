...
final class RustMacRendererTests: XCTestCase {
    ...
    func testRenderFrameCrossfadesBetweenTwoEnvelopes() throws {
        // Not a Core Graphics/Rust parity check (MacCanvasView has no
        // crossfade path at all) — exercises renderFrame's own optional
        // previousEnvelopeJSON/transitionElapsedMs parameters directly
        // (see rust/phpnitro-render/src/transition.rs).
        let renderer = try RustRenderer()
        let oldEnvelope = """
