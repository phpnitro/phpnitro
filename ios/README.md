...
**Fait depuis** : rendu réel des icônes (Core Text, `IconFont.swift`), hit-testing (`DrawCommandPayload.action(at:)`, câblé sur un vrai `UITapGestureRecognizer` dans `NativeCanvasView.swift`), parité de commandes de dessin avec `NativeCanvasView.kt` — `image` (`ImageLoader.swift`, décodage + cache + fetch réseau via `URLSession`, y compris les URIs `data:` d'une photo capturée/choisie), `spinner`/`skeleton` (animés via `CADisplayLink`), `clientPanel`/`hScroll`/`vScroll`/`slider` (rendu statique, contenu imbriqué via un dispatcher récursif `drawCommand(_:in:)`) —, une VRAIE boucle de fetch réseau (`ScreenClient.swift`, `NativeScreenViewController.swift`), et une vraie pile d'écrans : `ScreenNavigation.swift` (fonction pure `reduce(action:stack:)`, testée unitairement sans réseau ni UIKit) gère `navigate:`/`tab:`/`back`/`clientTab:` exactement comme le bloc `when` de tap-dispatch de `NativeRenderPocActivity.kt`. `NativeScreenViewController` a aussi un vrai écran d'erreur (`ScreenErrorView`, bouton "Réessayer") et transmet `fieldValues` sur chaque fetch (prêt côté protocole, mais aucun widget `TextField` n'existe encore côté iOS pour les remplir — voir point 3 plus bas).
...
4. Aucun overlay pour les Views natives (VideoPlayer, MapView, Lottie, TextField/EditText) — le pendant de `rootLayout.addView(...)` dans `NativeRenderPocActivity.kt`. C'est aussi ce qui bloque `fieldValues` : le champ existe côté protocole (`NativeScreenViewController.setFieldValue(_:forName:)`), mais aucun widget ne peut encore le remplir.
