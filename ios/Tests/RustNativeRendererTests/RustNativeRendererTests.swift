import XCTest
@testable import RustNativeRenderer

/// Real tests against the ACTUAL compiled rust/phpnitro-render library —
/// cross-compiled for the iOS Simulator target (aarch64-apple-ios-sim,
/// matching the Apple Silicon macos-14 CI runner's own simulator
/// architecture — see .github/workflows/ci.yml's ios-build job) and
/// linked at BUILD time via Package.swift's own linker flags. Unlike the
/// Linux/Windows ports there is no "library not found, skip" case to
/// guard here: if the Rust crate wasn't cross-compiled for real, the
/// whole package fails to LINK, not just this one test file.
final class RustNativeRendererTests: XCTestCase {
    private static func repoRoot(file: StaticString = #filePath) -> URL {
        // ios/Tests/RustNativeRendererTests/RustNativeRendererTests.swift
        // -> .../RustNativeRendererTests/ -> .../Tests/ -> .../ios/ -> repo root.
