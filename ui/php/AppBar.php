...
{
    private const DEFAULT_CLASSES = 'fixed top-0 left-0 right-0 z-10 flex items-center gap-3 px-4 h-14 '
    ...
    public static function make(string $title, ?string $backHref = null, string $classes = self::DEFAULT_CLASSES): self
    {
        return new self($title, $backHref, $classes);
    }

    public function render(): string
    {
        $back = '';
        if ($this->backHref !== null) {
            $back = sprintf(
