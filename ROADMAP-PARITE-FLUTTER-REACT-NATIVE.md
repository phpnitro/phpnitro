...
### 10. API de style typée très partielle
Seul `Text` a des paramètres typés (`TextSize`/`FontWeight`/`Color`) — tous les autres widgets n'acceptent que des classes Tailwind brutes en chaîne de caractères. Flutter a un système de thème cohérent et injectable (`ThemeData`) appliqué à tous les widgets ; ici, il n'y a qu'un bascule clair/sombre binaire, pas de thème personnalisable centralement (couleurs de marque, typographie custom, espacement cohérent).
