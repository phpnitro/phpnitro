...
| `hittest.rs` | Parcours `hitRegions` + `clientPanel`/`hScroll`/`vScroll` imbriqués, ordre premier-match-gagne confirmé en lisant le Kotlin directement ; `slider_hit_test()` séparé (`sliderRegions[]` — pas de `hitRegions` pour un slider, la valeur dépend d'où on tape) | 17 tests, cas adverses inclus + un test sur le vrai payload `sliderRegions` de `screen_widgets_forms.json` |
