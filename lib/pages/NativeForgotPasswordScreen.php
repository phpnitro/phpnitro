    ...
    public static function build(float $screenWidth, ?string $error, ?string $devResetLink): Widget
    {
        ...
        $rows = [
            ...
            new Padding(
                ...
                new Text(Translator::t('forgot_password.title'), Tokens::TEXT_DISPLAY - 2, Tokens::ink()->toHex(), bold: true),
            ),
            new Padding(
                EdgeInsets::only(top: 4),
                new Text(Translator::t('forgot_password.hint'), Tokens::TEXT_BODY_SMALL, Tokens::inkMuted()->toHex()),
            ),
            new Padding(EdgeInsets::only(top: Tokens::SPACE_LG), new Banner($error)),
            new Padding(EdgeInsets::only(top: Tokens::SPACE_XL), new TextField('username', placeholder: Translator::t('forgot_password.username_placeholder'))),
            new Padding(
                EdgeInsets::only(top: Tokens::SPACE_XL),
                new Button(Translator::t('forgot_password.submit'), 'submit:forgot_password', width: $contentWidth),
            ),
        ];
        ...
        if ($devResetLink !== null) {
            $rows[] = new Padding(
                EdgeInsets::only(top: Tokens::SPACE_LG),
                new Banner(Translator::t('forgot_password.dev_mode_banner', ['link' => $devResetLink]), icon: 'check_circle', background: Tokens::success(), foreground: Tokens::ink()),
            );
            $rows[] = new Padding(
                EdgeInsets::only(top: Tokens::SPACE_MD),
                new RichText([
                    new TextSpan(Translator::t('forgot_password.have_code'), color: Tokens::inkMuted()->toHex()),
                    new TextSpan(Translator::t('forgot_password.reset_link'), bold: true, color: Tokens::ink()->toHex(), action: 'navigate:reset-password'),
