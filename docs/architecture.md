...
`Engine\Native\Router` est une façon plus déclarative d'enregistrer un écran que de faire grandir le `match ($screen)` de `public/index.php` d'un cas de plus — `Router::register('product', fn () => ...)` puis `public/index.php` vérifie `Router::has($screen)` avant de retomber sur le `match()` historique. Les deux coexistent : ce n'est pas une migration des ~40 écrans existants (trop risqué en un seul passage), juste un point d'enregistrement plus propre pour un NOUVEL écran — voir `public/index.php` pour l'exemple réel (`product`, dont le cas dans `match()` a été retiré, pas dupliqué).
...
## Scroll imbriqué : un seul niveau, bubbling à la bordure

`NestedScroll` (`Canvas::verticalScroll()`) — une région avec sa PROPRE hauteur de viewport bornée, scrollable indépendamment du scroll de page qui la contient (un panneau "activité récente" plafonné à 120dp dans un écran par ailleurs plus long). Contrairement à `HorizontalScroll` (dont le scroll imbriqué se désambiguïse naturellement de celui de la page par l'AXE — horizontal contre vertical), ici les deux sont verticaux : `NativeCanvasView.kt` tranche par l'ESPACE plutôt — un drag qui démarre dans le rect de la région lui appartient, mais seulement jusqu'à son propre bord (haut/bas) ; passé ce bord, l'excédent du geste remonte vers le scroll de page pour le reste du même geste, exactement comme Flutter/RN. Un seul niveau de nesting reste suivi (une `NestedScroll` dans une autre n'est pas arbitrée), même contrainte que `HorizontalScroll` — voir le docblock de `verticalScroll()`.
