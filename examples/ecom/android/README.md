...
Mêmes mécanismes que `android/` à la racine (voir `../../../android/README.md` pour le détail : binaires PHP cross-compilés via le NDK, `PhpServer.kt`, pièges rencontrés) — ce dossier bundle simplement `examples/ecom/ui/` + `examples/ecom/backend/` au lieu de la démo du framework.

## Build & install

```bash
cd examples/ecom
bash bundle-android.sh          # copie ui/ + backend/ dans android/app/src/main/assets/www
cd android
